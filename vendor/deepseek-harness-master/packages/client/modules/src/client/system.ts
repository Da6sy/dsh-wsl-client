...
export class ClientModuleSystem implements ClientModuleLoader {
  ...
  /** The exact sink object installed on window, for identity-checked removal. */
  private readonly sink: NonNullable<DshWindow['__ModuleLoader__']>
  ...
  constructor(options: ClientModuleSystemOptions) {
    ...
    win.__ModuleLoader__ = this.sink
  }

  /**
   * Remove the `window.__ModuleLoader__` sink if it is still the one this
   * instance installed. Embedded hosts (e.g. a desktop shell) re-boot the
   * web shell inside a long-lived document; without this removal the next
   * constructor refuses with "already installed (double boot?)". The
   * identity check never removes a successor's sink.
   */
  uninstall(): void {
    const win = globalThis as DshWindow
    if (win.__ModuleLoader__ === this.sink) delete win.__ModuleLoader__
  }
