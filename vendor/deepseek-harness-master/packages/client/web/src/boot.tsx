...
export class AppWebEntry {
  ...
  /** Root fiber of the Loader plugin; disposing it cascades the whole tree. */
  private bootFiber: { dispose(): Promise<void> } | undefined
  /** Set by dispose(); stops an in-flight run() from settling afterwards. */
  ...
  /**
   * Tear the shell down far enough that a fresh {@link run} can boot in the
   * same document (the embedded desktop-shell case). Plain page-based hosts
   * never call this; they reload instead. Order matters: unmount the React
   * tree, then dispose the whole Cordis fiber tree (loader entries, fibers,
   * WebSocket connections, timers, listeners — every registration is a fiber
   * effect), and only then uninstall the window globals, so teardown effects
   * still see them.
   */
  async dispose(): Promise<void> {
    ...
    this.root?.unmount()
    this.root = undefined
    await this.bootFiber?.dispose()
    this.bootFiber = undefined
    // The module system owns the registration sink; removal is identity-checked.
    if (this.modules !== undefined) {
      this.modules.uninstall()
      const win = globalThis as DshWindow
      if (win.__DSH_MODULES__ === this.modules) delete win.__DSH_MODULES__
    }
  }
  ...
  private async runPluginBoot(prefetching: Promise<void>): Promise<void> {
    const ctx = this.ctx
    this.bootFiber = await ctx.plugin(Loader)
    if (this.disposed) {
      // Unmounted while boot was in flight: unwind the freshly mounted tree.
      await this.bootFiber.dispose()
      this.bootFiber = undefined
      return
    }
    ...
    // must then skip it).
    // Platform modules are already registered in the browser module table and
    // are namespaces, not Cordis plugins. Older or mixed dsh installations
    // may still advertise one of them in the server graph; creating a loader
    // entry for such a row produces "invalid plugin" (for example,
    // ui-attachment only exports React components). Keep those rows out of
    // Cordis while retaining the real client plugin entries.
    const staticModuleIds = new Set(Object.keys(getStaticModules()))
    const rows = [...new Set([
      MODULES_ID,
      ...this.manifest.plugins
        .map(row => row.id)
        // app-shell is owned and registered by this kernel. A server graph
        // from a mixed dsh installation may advertise it as well; creating
        // that second entry calls slots.install() twice and fails boot.
        .filter(id => id !== MODULES_ID && id !== APP_SHELL_ID && !staticModuleIds.has(id)),
      APP_SHELL_ID,
    ])]
