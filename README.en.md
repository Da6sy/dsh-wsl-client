...
An **Electron** desktop application that embeds the official **DeepSeek Harness (`dsh`)** Web Shell natively in a standalone window, and adds **multi-tabs** plus a **built-in terminal (WSL / local pseudo-terminal)** in the same interface.
...
- Tabs support drag-to-reorder, active highlight, hover-revealed close button, middle-click close, and automatic compression + horizontal scrolling when there are many tabs.
...
└─ src/
   ...
   └─ renderer-react/
      ...
      ├─ tabbar.jsx           # tab strip + new-tab menu
