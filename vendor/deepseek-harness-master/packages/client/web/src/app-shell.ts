...
export function apply(ctx: Context): void {
  // A duplicate app-shell entry must not replace the service owned by the
  // first shell entry. This also prevents a second reflect.provide() from
  // failing after the renderer's boot-once check has been handled.
  if (ctx.get('appShell') !== undefined) return
  ...
  try {
    ...
    // A second shell copy can come from a mixed dsh manifest or package
    // graph. The existing renderer is already the one this shell needs.
    if (!String(error).includes('slot renderer already installed')) throw error
  }
